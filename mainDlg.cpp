...
{
	CString url;
	url = _T("https://bdpbx.com/softphone-update.txt");
	...
	if (response->statusCode == 200) {
		if (!response->body.IsEmpty() && response->body.Left(4) == "http") {
			...
			if (pos > 0) {
				...
				CString strObject;
				INTERNET_PORT nPort;
				if (AfxParseURL(CString(url), dwServiceType, strServer, strObject, nPort) && dwServiceType == AFX_INET_SERVICE_HTTPS && strServer.Right(10) == _T(".bdpbx.com")) {
					allowed = true;
				}
